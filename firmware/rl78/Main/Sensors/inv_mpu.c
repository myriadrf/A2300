...
/******************************** RL78 ********************************/
#elif defined(MOTION_DRIVER_TARGET_RL78)
/* Loctronix,LLC */
#include <math.h>
...
#define log_i  printf  // Log Informational
#define log_e  printf  // Log error.
#define delay_ms(A)  Timer_Delay((A)*1000)
#define min(A,B)   (((A)<=(B))?(A):(B))
// Get millisecond counter.
static void get_ms(unsigned long *pms)
{
	Timer_GetTime(pms);
}
// Initialize Callback.
...
{
    // This function defined in motion.c
	extern void sensor_board_irq_connect(unsigned short pin, void (*cb)(void), void *arg);
...
#ifndef MPU6500
#define MPU6500					/* Combines MPU-6500 */
...
#if !defined(MOTION_DRIVER_TARGET_RL78)
const struct gyro_reg_s reg = {
    .who_am_i       = 0x75,
    .rate_div       = 0x19,
    .lpf            = 0x1A,
...
#elif defined(MOTION_DRIVER_TARGET_RL78)
// RL78 Compiler does not allow for "." member initializations.
const struct gyro_reg_s reg = {
    /*.who_am_i       =*/0x75,
    /*.rate_div       =*/0x19,
    /*.lpf            =*/0x1A,
...
const struct hw_s hw = {
    /*.addr           =*/ 0xD0, /* Loctronix i2c uses 0xD0 (0x68 shifted) */
    /*.max_fifo       =*/ 1024,
    /*.num_reg        =*/ 118,
    /*.temp_sens      =*/ 340,
    /*.temp_offset    =*/ -521,
...
#endif
static unsigned char has_accel = 1;
#endif  // MPU6500
static struct gyro_state_s st;
...
int mpu_init(struct int_param_s *int_param)
{
	...
	st.hw = &hw;
	st.test = &test;
	// CJC: Workaround to allow mpu_read_mem() to succeed.
	st.chip_cfg.sensors = INV_XYZ_GYRO | INV_XYZ_ACCEL; // 0xFF;
...
#define MPU6500_MEM_REV_ADDR    (0x17)
    ...
    if (rev == 0x1)
        st.chip_cfg.accel_half = 0;
	else if (rev == 0x2)		// Loctronix.
        st.chip_cfg.accel_half = 1;  // CJC
    else {
        log_e("Unsupported software product rev %d.\n", rev);
        return -1;
    }
...
int mpu_set_accel_bias(const long *accel_bias)
{
    ...
    data[0] = (unsigned char)( (accel_hw[0] >> 8) & 0xff );
    data[1] = (unsigned char)( (accel_hw[0]) & 0xff );
    data[2] = (unsigned char)( (accel_hw[1] >> 8) & 0xff );
    data[3] = (unsigned char)( (accel_hw[1]) & 0xff );
    data[4] = (unsigned char)( (accel_hw[2] >> 8) & 0xff );
    data[5] = (unsigned char)( (accel_hw[2]) & 0xff );
    ...
    if (st.chip_cfg.dmp_on)
        ...
        if (st.chip_cfg.lp_accel_mode) {
            if (rate && (rate <= 40)) {
                ...
                mpu_lp_accel_mode((byte)rate);  // CJC: Warning on function(byte).
                return 0;
            }
...
int mpu_set_compass_sample_rate(unsigned short rate)
{
    ...
    div = (unsigned char)( st.chip_cfg.sample_rate / rate - 1 );
...
//    log_i("FIFO count: %hd\n", fifo_count);
    ...
    if (i2c_read(st.hw->addr, st.reg->fifo_r_w, packet_size, data))
        return -1;
    more[0] = (byte) (fifo_count / packet_size - 1);
    sensors[0] = 0;
    ...
    unsigned char *more)
{
    ...
    if (i2c_read(st.hw->addr, st.reg->fifo_r_w, length, data))
        return -1;
    more[0] = (uint8_t)( fifo_count / length - 1 );
    return 0;
}
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    fifo_count = (data[0] << 8) | data[1];
    packet_count = (byte) (fifo_count / MAX_PACKET_LENGTH);
...
#define LOAD_CHUNK  (16)
    ...
    tmp[0] = (uint8_t)( start_addr >> 8 );
    tmp[1] = (uint8_t)( start_addr & 0xFF );
...
static int setup_compass(void)
{
    ...
    if (i2c_read(st.chip_cfg.compass_addr, AKM_REG_ASAX, 3, data))
        return -1;
    st.chip_cfg.mag_sens_adj[0] = (short)data[0] + 128;
    st.chip_cfg.mag_sens_adj[1] = (short)data[1] + 128;
    st.chip_cfg.mag_sens_adj[2] = (short)data[2] + 128;
...
int mpu_get_compass_reg(short *data, unsigned long *timestamp)
{
    ...
    data[0] = (byte) (((short)data[0] * st.chip_cfg.mag_sens_adj[0]) >> 8);
    data[1] = (byte) (((short)data[1] * st.chip_cfg.mag_sens_adj[1]) >> 8);
    data[2] = (byte) (((short)data[2] * st.chip_cfg.mag_sens_adj[2]) >> 8);
    ...
    unsigned char lpa_freq)
{
    ...
    if (lpa_freq) {
        ...
        if (thresh > 1020)
            ...
            thresh_hw = (unsigned char)( thresh >> 2 );
#endif
