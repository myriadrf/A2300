...
{ 	uint16  idMsg = (phdr->idCategory << 8) | phdr->idType;
	return idMsg == id;
}
...
void   Dci_Hdr_SetSeq1(Dci_Hdr* phdr, byte sndSeq, byte ackSeq) { phdr->seq = ( ((sndSeq&0xF) << 4) | (ackSeq&0xF)); }

byte   Dci_Hdr_GetSeqAck(Dci_Hdr* phdr) 				{ return phdr->seq & 0xF;}
void   Dci_Hdr_SetSeqAck(Dci_Hdr* phdr, byte acqSeq)  { phdr->seq = (phdr->seq & 0xF0) | (acqSeq & 0x0F);}

byte   Dci_Hdr_GetSeqSnd(Dci_Hdr* phdr) 				{ return ((phdr->seq & 0xF0) >> 4);}
void   Dci_Hdr_SetSeqSnd(Dci_Hdr* phdr, byte acqSnd) { phdr->seq = (phdr->seq & 0x0F) | (acqSnd << 4);}
