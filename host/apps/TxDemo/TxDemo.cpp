...
static DataStreamWriter s_s1(4, 1024*64, 8, '.');
//static DataStreamWriter s_s2(4, 1024*64, 16, '2');

/*****************************************************************
* Forward Declarations.
*****************************************************************/

static int  TransmitData (size_t msecDur);
...
//	s_s2.RegisterArgs( "s2", s_args);
	...
	if( retval >= 0)
	{
		...
		if( retval == 0)
		{
			...
			retval = TransmitData((size_t) (s_args.GetDouble("duration")*1000.0));
		}
...
static int TransmitData (size_t msecDur)
{
	...
	//Start the Writers.
	retval = s_s1.Start(msecDur, true);  //Starts but does not enable.
